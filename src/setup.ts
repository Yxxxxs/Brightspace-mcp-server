...
async function main(): Promise<void> {
  ...
  console.log(dim("  By Yousef Alami — github.com/Yxxxxs/Brightspace-mcp-server"));
  console.log("");
  ...
  if (preset) {
    ...
    while (!baseUrl) {
      const raw = await ask(
        rl,
        "What is your Brightspace URL? (e.g., brightspace.yourschool.edu): ",
      );
      const normalized = normalizeUrl(raw);
      if (!raw || !isValidUrl(normalized)) {
        console.log(yellow("  Please enter a valid URL (e.g., brightspace.yourschool.edu)"));
        continue;
      }
      baseUrl = normalized;
    }
